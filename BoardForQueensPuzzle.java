...
public class BoardForQueensPuzzle {
    ...
    public String toString() {
        ...
        // short-circuit the representationture for a zero-sized board
        if( size == 0) return representation;

        representation += INDENT + "files" + System.lineSeparator()
            +  INDENT;
        String underscores = "";

        // file numbers, each in a 3-column field
        for( int file = 0; file < size; file++) {
            representation += String.format("%-3d", file);
        ...
        // representationture each rank
        for( int rank = 0; rank < size; rank++){
            representation += // right-justified rank number:
            ...
            for( int file = 0; file < size; file++)
                if(    rank > lastRankFilled      // no queen in this rank yet
                    || filesWithQueens[ rank] != file) // no queen in this file
                     representation += " _ ";
                else representation += " Q ";
            representation += System.lineSe
parator();
        }
